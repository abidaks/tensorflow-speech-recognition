...
from tensorflow import audio as contrib_audio
...
class AudioProcessor(object):
  ...
  def get_data(self, how_many, offset,
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      if how_many == -1 or pick_deterministically:
        ...
        if np.random.uniform(0, 1) < pseudo_frequency and len(pseudo_candidates) > 0:
          #print(pseudo_candidates)
          #print(len(pseudo_candidates))
          #print(np.random.randint(len(pseudo_candidates)))
  ...
  def summary(self):
    ...
    print("1%% <-> %d samples in 'validation'" %
          int(self.set_size('validation') / 100))
    print("1%% <-> %d samples in 'testing'" %
          int(self.set_size('testing') / 100))
